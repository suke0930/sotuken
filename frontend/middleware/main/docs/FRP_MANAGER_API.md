...
sequenceDiagram
    ...
    Client->>MW: POST /api/frp/auth/init<br/>{ fingerprint }
    ...
    Client->>MW: GET /api/frp/auth/poll?tempToken=...
    MW->>AuthJS: GET /api/auth/poll
...
| メソッド | パス | 役割 |
| --- | --- | --- |
| `POST` | `/api/frp/auth/init` | Discord 認証セッション開始（`fingerprint` 必須） |
| `GET` | `/api/frp/auth/poll` | 認証状態をポーリング (`tempToken` クエリ) |
| `GET` | `/api/frp/auth/status` | 現在の JWT/ユーザー状態を確認 |
...
curl -X POST http://localhost:12800/api/frp/auth/init \
  ...
  -d '{"fingerprint":"fp_localhost_dev"}'

# 2) 認証 URL をブラウザで開いた後、ポーリング
curl "http://localhost:12800/api/frp/auth/poll?tempToken=PASTE_TEMP_TOKEN"
...
### トラブルシュートの目安
- `fingerprint is required`: `/api/frp/auth/init` に fingerprint が無い。
